...
const ApplicationPdfViewer = ({
  ...
  const [responses, setResponses] = useState<{ value: string | null; form_fields: FormField | null; }[]>([]);
  ...
  const fetchResponses = async () => {
    const { data, error } = await supabase
      ...
      .select(`value, form_fields ( id, label, field_type, options, is_required, order, display_rules, help_text, description, tooltip, form_id, section_id )`) // Added form_id, section_id
      .eq('application_id', applicationId);

    if (error) {
      console.error("Error fetching application responses for PDF:", error);
      showError("Failed to load application responses for PDF.");
      return [];
    }
    return data.map(res => ({
      ...res,
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      setResponses(fetchedResponses); // Set responses for rendering

      // Wait for the next render cycle to ensure responses are in the DOM
      await new Promise(resolve => setTimeout(resolve, 50)); 

      if (!pdfContentRef.current) {
        showError("PDF content not found after rendering.");
        return;
      }
      ...
      setResponses([]); // Clear responses after PDF generation to hide content
    }
  };
  ...
  return (
    <>
      ...
      {responses.length > 0 && ( // Only render if responses are loaded for PDF generation
