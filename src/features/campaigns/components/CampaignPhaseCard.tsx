...
interface CampaignPhaseCardProps {
  phase: CampaignPhase;
  index: number;
  onEdit: (phase: CampaignPhase) => void;
  onDelete: (phaseId: string) => void;
  onConfigure: (phase: CampaignPhase) => void;
  canEditOrDelete: boolean;
}

export function CampaignPhaseCard({ phase, index, onEdit, onDelete, onConfigure, canEditOrDelete }: CampaignPhaseCardProps) {
  return (
                  ...
                  <AlertDialogContent className="rounded-xl shadow-lg bg-card text-card-foreground border-border">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-body-medium text-muted-foreground">
                        Are you sure you want to delete the campaign phase &quot;{phase.name}&quot;? This action cannot be undone.
