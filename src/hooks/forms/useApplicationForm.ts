    ...
    defaultValues: useMemo(() => {
      ...
      formFields.forEach(field => {
        if (field.field_type === 'checkbox') {
          ...
          defaults[field.id] = field.rating_min_value ?? 1; // Default to min value for rating
        } else {
          defaults[field.id] = '';
        }
      });
    ...
    const filtered = formFields.filter(field => {
      ...
      if (!shouldDisplay) {
        ...
        if (field.field_type === 'checkbox') {
          ...
          newValue = (field.is_required ? (field.rating_min_value ?? 1) : undefined);
        } else {
          newValue = '';
        }
