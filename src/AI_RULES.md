...
# Global Gateway Terminology and RBAC Model

This section defines key terms and the Role-Based Access Control (RBAC) model for the "Global Gateway" platform.

## Core Modules:
- **Form Builder**: Module for designing custom application forms.
- **Email Message Composer**: Module for creating and managing reusable email templates.
- **Notifications**: System for informing users about important events (in-app, email).
- **Workflow Builder**: Module for defining sequential stages and automated actions for applications.
- **Review**: Module for evaluating submitted applications.
- **Programs**: Top-level module for managing grants, funding, awards, or job applications.

## Key Entities:
- **Program**: A specific opportunity (e.g., "Innovators Scholarship") that users can apply for. Each program has an associated application form and workflow.
- **Application (Submission)**: A user's completed submission for a specific Program.
- **Form**: A collection of sections and fields used to gather information from applicants. Forms can be created from scratch or from templates.
- **Form Section**: A logical grouping of fields within a Form.
- **Form Field**: An individual input element within a Form (e.g., text input, dropdown, checkbox).
- **Workflow**: The defined sequence of stages an Application progresses through.
- **Program Stage**: A specific step within a Program's Workflow (e.g., "Submitted", "Under Review", "Accepted").
- **Email Template**: A reusable email message with a subject and body, often containing dynamic placeholders.
...
1.  **Role: Applicant**
    ...
    *   **Key Restrictions**: Cannot see internal details like reviewer notes, scores, or who is assigned to their application.

2.  **Role: Recommender**
    *   **Core Purpose**: To provide a recommendation for a specific applicant via a unique link.
    *   **Key Permissions**: Can view and fill out the single recommendation form they were invited to.
    *   **Key Restrictions**: No login or account required. Access is limited to the single-use, secure link. Cannot view the main application or any other part of the platform.

### Part 2: Internal Team Roles

This is the updated hierarchy for your internal team, from most restricted to most powerful.
